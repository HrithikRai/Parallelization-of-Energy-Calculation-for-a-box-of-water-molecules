// waters.c OPTION 2- SEQUENTIAL READING OF DATA BY INDIVIDUAL THREADS.
#include <stdio.h>
#include <math.h>
#include <stdbool.h>
#include <stdlib.h>
...
main(int argc,char **argv){
  ...
  MPI_Bcast(&name,8,MPI_CHAR,0,MPI_COMM_WORLD);
  for(i=0; i<num_procs;i++)
  {
      if(rank==i)
      {
        printf("I am thread nummer %d and received the name %s\n",rank,name);
  ...
  // PAIRS REDUCTION
  long minnpair = 0;
  MPI_Reduce(&npair,&minnpair,1,MPI_LONG,MPI_MIN,0,MPI_COMM_WORLD);
  long maxnpair = 0;
  MPI_Reduce(&npair,&maxnpair,1,MPI_LONG,MPI_MAX,0,MPI_COMM_WORLD);
